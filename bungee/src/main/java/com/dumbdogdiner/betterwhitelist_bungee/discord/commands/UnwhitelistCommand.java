    ...
    public void run(MessageReceivedEvent e, String... args) {
        ...
        if (e.getMessage().getMentionedMembers().size() > 0) {
            ...
            if (!Objects.requireNonNull(e.getMember()).canInteract(member)) {
                e.getChannel()
                        .sendMessage(":x: You do not have the required permissions to remove this user from the whitelist.")
                        .queue();
                return;
            }
            target = member;
        ...
        if (target == null) {
            e.getChannel()
                .sendMessage(":x: Unable to find user.")
                .queue();
            return;
        }

        if (SQLConnection.removeEntry(target.getId())) {
            e.getChannel()
                .sendMessage(":white_check_mark: User `"+ target.getUser().getAsTag() +"` was removed from the whitelist.")
                .queue(message -> RoleUtil.removeGrantedRole(e));
        } else {
            e.getChannel()
                .sendMessage(":x: **Could not remove user from the whitelist!** Perhaps they aren't on it to begin with?")
                .queue();
        }
    }
}
